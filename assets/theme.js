...
/* Product card */
class CardCountdownTimer extends HTMLElement {
  constructor() {
    super();
...
customElements.define('card-countdown-timer', CardCountdownTimer);
...
class RevealBanner extends HTMLElement {
  ...
  init() {
    ...
    const isLandscape = this.classList.contains('reveal-banner--landscape-3-2');
   
    if (isLandscape) {
    Motion.scroll(
      Motion.animate(this.querySelectorAll('.banner__media'), { clipPath: ['inset(10% 5% 0% 5% round var(--rounded-block))', 'inset(0% 0%)'] }, { easing: 'linear' }),
 ...
 Motion.scroll(
    ...
    if (tracker2nd) {
      ...
      ]), { target: tracker2nd, offset: ["start 480%", "end 150%"] });    
...
class ProductBundle extends HTMLElement {
  ...
  init() {
    this.detailsContent.forEach((detail,index) => {
      if(!detail.dataset.url){
        return;
      }
      fetch(detail.dataset.url)
      .then(response => response.text())
      .then(responseText => {
        const sectionInnerHTML = new DOMParser()
          .parseFromString(responseText, 'text/html')
        ...
        const detailsContent = sectionInnerHTML.querySelectorAll('product-bundle .details__content');
        if (detailsContent && detailsContent[index] && detailsContent[index].innerHTML.trim().length) {
          this.detailsContent[index].innerHTML = detailsContent[index].innerHTML;
          this.detailsContent[index].dispatchEvent(new CustomEvent('recommendations:loaded'));
        }
        else {
          this.detailsContent[index].closest(".product__accordion").remove();
          this.detailsContent[index].dispatchEvent(new CustomEvent('is-empty'));
        }



        this.bindSubmit.bind(this)()
      })
      .catch(e => {
        console.error(e);
      });
    })
    this.productForm = this.closest(".product__info").querySelector(".product-form")
    this.initEG.bind(this)()
  }
  initEG(){
    const targetContent = this.querySelector(".product__accordion:first-child .details__content");
    fetch("".concat("https://aca.506.io", "/public/store?shop=").concat(window.Shopify.shop)).then(response => response.json())
      .then(responseJSON => {
        const rules = responseJSON.rules
        let allProducts = []
        rules.forEach(rule => {
          const products = rule.action.products
          const variantIds = rule.trigger.products.map(obj => obj.variantId + "")
          const productVariantId = this.productForm.querySelector("input[name='id']").value +"";

          if(variantIds.includes(productVariantId)){
           allProducts = allProducts.concat(products)
           Promise.all(products.map((e) => { return fetch("/products/".concat(encodeURIComponent(e.handle), ".json")).then(response => response.json()).then(productJSON => {
              // const variants = productJSON.product.variants
             this.querySelector("[for='Recommendations_"+e.handle+"'] img").src =  productJSON.product.image.src
             this.querySelector("[for='Recommendations_"+e.handle+"'] .product-bundle__title").innerText =  productJSON.product.title
             this.querySelector("[for='Recommendations_"+e.handle+"']").style.display = 'flex'
           })
          }))
         }
        })
       // add product
        allProducts.forEach(item => {
          if(targetContent){
            targetContent.insertAdjacentHTML("afterbegin",`<label style="display:none;" class="product-bundle__info" for="Recommendations_${item.handle}"><img  src="" alt="${item.name}" width="50" height="50" loading="lazy" is="lazy-image"><div class="product-bundle-inner"><div class="product-bundle__title"></div><div class="price price--on-sale flex items-end gap-2 text-sm"><span class="sr-only">Sale price</span><span class="price__regular whitespace-nowrap  price__free " data-price="0">FREE</span></div></div><input name="bundles" value="{item.variantId}" class="checkbox unsubmit" id="Recommendations_${item.handle}" type="checkbox" checked disabled></label>`)
          }
        })
    }).catch(function(e) {
        console.log(e)
  ...
  bindSubmit(){
    const bundles = this.querySelectorAll("input[name='bundles']");
    bundles.forEach(bundle => {
      bundle.addEventListener("change", () =>{
        var bundlePrice = 0
        const bundlesChecked = this.querySelectorAll("input[name='bundles']:checked:not(.unsubmit)");
        bundlesChecked.forEach(bc => {
        ...
        productForms.forEach((productForm) => {
          ...
          if( window.Currency.currency_code_enabled){
            addButtonPrice.innerText = window.Currency.money_with_currency_format.replace("{{amount}}",formatWithDelimiters(price) )
          }else{
            addButtonPrice.innerText = window.Currency.money_format.replace("{{amount}}",formatWithDelimiters(price) ) 
          }
